...
To feed the simulation front-end with custom image data:
    ...
    - Linux users must specify the length of the path locating the image data file.
    	...
    	(<user folder>/CoaxlinkQuadCxp12_1cam/04_ref_design/sim/Simulation_FileIO_pkg.vhd)
    ...
    <user folder>/CoaxlinkQuadCxp12_1cam/04_ref_design/sim/ImageData_256x10_Mono8.dat
    ...
    - Execute the 'FrameRequest' command inside the 'Simulation' process of the SimulationCtrl_tb
      (<user folder>/CoaxlinkQuadCxp12_1cam/04_ref_design/sim/SimulationCtrl_tb.vhd):
